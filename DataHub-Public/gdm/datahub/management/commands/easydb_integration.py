...
class Command(BaseCommand):
    ...
    def checksum_to_json(self, dataset):
        ...
        datasets_df = pd.read_csv(checksum_path, sep='\s+', names=["checksum","filename"])
        metadata_dict = datasets_df.to_dict(orient="records")
        metadata_json = json.dumps(metadata_dict)
        jsons = list()
        for index, row in datasets_df.iterrows():
            if 'json' in row['filename']:
                jsons.append(row['filename'])
        merged_dict = dict()
    ...
    def validate_data(self, json_object, validator = {'nuri': 'http://coll\.mfn-berlin\.de/[\w/._]+$'}):      
    ...
    def validate_file(self, filename, file_path, validation = {}):
        # pass
    ...
    def export_object(self,files_path, affiliatedproject_object, json_object):
        ...
        if self.dataset_json[json_object]:
            ...
            for image in image_list:
                ...
                validation_dict = self.validate_file(filename, file_path, image_validators)
                ...
                if dataset_object:
                    ...
                    if self.attachment:
                        scheduler.add_job(self.export_dataset,
                                        replace_existing=True, id="easydb_export_"+ self.dataset_object.files)
                        # # scheduler.logger()
                        scheduler.start()
