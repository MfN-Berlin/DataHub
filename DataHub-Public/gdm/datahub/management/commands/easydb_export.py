        ...
        if response.status_code == 200:
            if len(response.json()["objects"]) > 0:
                ...
                if valid_GIN:
                    ...
                    if 'knochen' in self.params[2].lower():
                        new_body[0]["specimen"]["collection"] = {"_objecttype": "subjectheadings",
                            "_mask": "subjectheadings__all_fields",
                            "_global_object_id": "8640@63aa8bb3-6981-4c2d-8774-4c8f7931abd0",
                            "subjectheadings": {"_id": 120}}
            ...
            for filename in file_list:
                ...
                if media_dict:
                    ...
                    if 'coll.' in filename:
                        ...
                        if object_NURI:
                            ...
                            if catalog_no:
                                ...
                                specimen_object = self.specimen_handler(
                                    catalog_no)
                    ...
                    if response.status_code == 200:           # TODO: write else condition
                        ...
                        self.logger.info(
                            'Import error: {}'.format(response.text))
                else:
                    self.logger.info('No upload for media {}'.format(filename))
    ...
    def handle(self, *args, **options):
        ...
        if is_auth:
            ...
            scheduler.add_job(self.export_data,
                              replace_existing=True, id="easypeasy")
            scheduler.start()
            # self.export_data()
        else:
            self.logger.error(
